...
@Component({
    ...
    <div class="cc-dashboard">
      ...
      <div class="dashboard-grid" *ngIf="metrics">
        ...
        <mat-card class="chart-card">
          ...
          <mat-card-content>
            <ng-container *ngTemplateOutlet="pieChart; context: { rows: changeClassificationPieRows() }"></ng-container>
          </mat-card-content>
        </mat-card>
        ...
        <mat-card class="chart-card">
          ...
          <mat-card-content>
            <ng-container *ngTemplateOutlet="pieChart; context: { rows: changeTypePieRows() }"></ng-container>
          </mat-card-content>
        </mat-card>
        ...
        <mat-card class="chart-card">
          ...
          <mat-card-content>
            <ng-container *ngTemplateOutlet="pieChart; context: { rows: changeDepartmentPieRows() }"></ng-container>
          </mat-card-content>
        </mat-card>
        ...
        <mat-card class="chart-card wide">
          ...
          <mat-card-content>
            <ng-container *ngTemplateOutlet="pieChart; context: { rows: changeTrendPieRows() }"></ng-container>
          </mat-card-content>
        </mat-card>
        ...
        <mat-card class="chart-card">
          ...
          <mat-card-content>
            <ng-container *ngTemplateOutlet="pieChart; context: { rows: changePriorityPieRows() }"></ng-container>
          </mat-card-content>
        </mat-card>
      </div>
  ...
  styles: [`
    ...
    .metrics-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(128px, 1fr)); gap: 8px; margin-bottom: 14px; }

    .metric-card { position: relative; display: grid; place-items: center; padding: 8px 8px 7px; min-height: 58px; }

    .metric-icon-wrapper { position: absolute; top: 7px; left: 7px; width: 28px; height: 28px; border-radius: 7px; display: flex; align-items: center; justify-content: center; }
    ...
    .metric-body { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; width: 100%; min-width: 0; padding: 0 18px; }
    ...
    .dashboard-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 16px; margin-bottom: 24px; }
...
export class CcDashboardComponent implements OnInit {
  ...
  canReadChange = hasStoredPermission('CHANGE_CONTROL', 'READ', 'change_request');
  ...
  changeClassificationPieRows(): PieRow[] {
    return (this.metrics?.byClassification || []).map((item) => ({
      label: this.formatType(item.classification),
      value: item.count,
    }));
  }

  changePriorityPieRows(): PieRow[] {
    return (this.metrics?.byPriority || []).map((item) => ({
      label: this.formatType(item.priority),
      value: item.count,
    }));
  }

  changeTypePieRows(): PieRow[] {
    return (this.metrics?.byType || []).map((item) => ({
      label: this.formatType(item.type),
      value: item.count,
    }));
  }

  changeDepartmentPieRows(): PieRow[] {
  ...
  changeTrendPieRows(): PieRow[] {
    const trendData = this.metrics?.trendData || [];
    return [
      { label: 'Submitted', value: trendData.reduce((sum, item) => sum + item.submitted, 0) },
      { label: 'Closed', value: trendData.reduce((sum, item) => sum + item.closed, 0) },
      { label: 'Rejected', value: trendData.reduce((sum, item) => sum + item.rejected, 0) },
    ];
  }
  ...
  downloadCycleTimeReport(): void {
    if (!this.metrics) return;
    this.downloadFile(
      'change-control-cycle-time-report',
      'json',
      JSON.stringify({
        reportTitle: 'Change Control Cycle Time Report',
  ...
  exportRegister(): void {
    this.ccService.getChangeRequests().subscribe({
      next: (rows) => {
        this.downloadFile('change-control-register-export', 'csv', this.toCsv(rows.map((row) => ({
          ...
          owner: row.changeOwnerName,
          department: row.department,
          requestedDate: this.formatDate(row.requestedDate),
          targetImplementationDate: this.formatDate(row.targetImplementationDate),
          validationRequired: row.validationRequired,
          trainingRequired: row.trainingRequired,
        }))), 'text/csv');
      },
      error: () => this.snackBar.open('Unable to export change register', 'Dismiss', { duration: 4000 }),
    });
  }
