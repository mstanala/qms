...
@Component({
  selector: 'dev-e-signature-dialog',
  standalone: true,
...
export class ESignatureDialogComponent {
  ...
  sign(): void {
    ...
    // In production, this would validate the password against the backend
    // For now, we accept any non-empty password to allow testing
    setTimeout(() => {
      this.signing = false;
